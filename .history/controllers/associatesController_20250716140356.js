...
exports.addUser = async (req, res) => {
  ...
  try {
    ...
    const lastAssociate = await Associate.findOne({ associateId: { $exists: true } }).sort({ associateId: -1 });
    const associateId = (lastAssociate && lastAssociate.associateId) ? lastAssociate.associateId + 1 : 1;
    
    const associate = new Associate({ name, email, invitationLink, status: 'pending', associateId, invitationCode: invitationToken });
...
exports.addClients = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Client invitation sent successfully.', client });
  } catch (error) {
...
exports.getClientRequests = async (req, res) => {
  try {
    const clients = await Associate.find({ status: { $in: ['accepted', 'rejected', 'pending'] } });
