...
exports.addContracts = async (req, res) => {
  ...
  if (!contractName || !contractType) {
    return res
      .status(400)
      .json({ message: "Contract name and type are required." });
  }
  try {
    // Generate sequential contractId
    const lastContract = await Contract.findOne({ contractId: { $exists: true } }).sort({ contractId: -1 });
    const contractId = (lastContract && lastContract.contractId) ? lastContract.contractId + 1 : 1;
...
exports.uploadSignature = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Signature uploaded successfully.", contract });
...
exports.shareContract = async (req, res) => {
  ...
  if (!contractId || !associateName || !associateEmail || !sharedBy) {
    return res
      ...
      .json({
        message:
          "Contract ID, associate name, email, and sharedBy are required.",
      });
  }
  try {
    const contract = await Contract.findOne({ contractId: contractId });
    ...
    res
      .status(200)
      .json({ message: "Contract shared successfully.", contract });
...
exports.getAllContracts = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Contracts retrieved successfully.", contractId, contracts });
  } catch (error) {
