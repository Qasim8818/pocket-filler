...
exports.signup = async (req, res) => {
  ...
  try {
    ...
    console.log('Signup verification code:', verificationCode, 'Expires at:', verificationCodeExpires);
...
exports.organizationLogin = async (req, res) => {
  ...
  try {
    const organization = await Auth.findOne({ email, isOrganization: true });
    if (!organization) {
    ...
    const token = jwt.sign(
      { userId: organization._id, email: organization.email, roles: organization.roles, isOrganization: true },
      process.env.SECRETKEY,
      { expiresIn: '1h' }
    );

    console.log(`Organization ${organization.email} logged in with roles: ${organization.roles ? organization.roles.join(', ') : 'none'}`);
    ...
    res.status(500).json({ message: 'An error occurred during organization login.' });
  }
};
...
exports.verifySignupCode = async (req, res) => {
  ...
  if (!email || !code) {
    return res.status(404).json({ message: 'Email and verification code are required.' });
  }
  ...
  try {
    ...
    if (!user.verificationCodeExpires || user.verificationCodeExpires < new Date()) {
      ...
      return res.status(409).json({ message: 'Verification code has expired.' });
    }
