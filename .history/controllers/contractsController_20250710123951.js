...
// Multer setup for contract file uploads
...
// Add contract with name and type
...
// Preview contract (return contract details and file path)
...
// Save contract (finalize contract)
...
// Share contract with associate
exports.shareContract = async (req, res) => {
  const { contractId, associateName, associateEmail } = req.body;
  if (!contractId || !associateName || !associateEmail) {
    return res.status(400).json({ message: 'Contract ID, associate name, and email are required.' });
  }
  try {
    const contract = await Contract.findById(contractId);
    if (!contract) {
    ...
    // Add associate if not already added
    const exists = contract.associates.some(a => a.email === associateEmail);
    if (!exists) {
      contract.associates.push({ name: associateName, email: associateEmail });
      await contract.save();
    }
